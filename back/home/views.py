...
class FindRoomView(APIView):
    ...
    def post(self, request, format=None):
        ...
        enough_capacity_rooms = RoomModel.objects.filter(id__in=available_rooms, capacity__gte=persons).order_by('capacity')
        if enough_capacity_rooms.exists():
            rooms_json = serializers.serialize('json', enough_capacity_rooms)
            print(rooms_json)
            #ReservationModel.objects.create(guest_id=guest_instance,room_id=enough_capacity_rooms[0], res_from=checkInDate_object, res_until=checkOutDate_object, people=persons)
...
@login_required
def get_guest_reservations(request):
    guest = request.user;
    reservations = ReservationModel.objects.filter(guest_id=guest.id).values_list('room_id', flat=True)
    guest_reservations = ReservationModel.objects.filter(guest_id=guest.id).order_by('res_from')
    reserved_rooms = RoomModel.objects.filter(id__in=reservations)
    data = {
        'reservations': serializers.serialize('json', guest_reservations),
        'reserved_rooms': serializers.serialize('json', reserved_rooms)
    }
...
def create_reservation(request):
    ...
    print(serializer)
    #ReservationModel.objects.create(guest_id=guest,room_id=room, res_from=datetime.now(), res_until=datetime.now(), people=3)
    return Response({"Reservation created"}, status=status.HTTP_201_CREATED)
